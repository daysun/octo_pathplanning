...
namespace octomap {
  ...
  public:
    bool has_project;
    bool setUpdatePro;
    ColorOcTreeNode() : OcTreeNode(),has_project(false),setUpdatePro(false) {}

    ColorOcTreeNode(const ColorOcTreeNode& rhs) : OcTreeNode(rhs), color(rhs.color),id(rhs.id)
      ,has_project(rhs.has_project),setUpdatePro(rhs.setUpdatePro) {}

    bool operator==(const ColorOcTreeNode& rhs) const{
      return (rhs.value == value && rhs.color == color && rhs.id == id && rhs.has_project == has_project && rhs.setUpdatePro == setUpdatePro);
    }
